...
const PAGE_META = {
  ...
  '/system/account': { title: '账号权限管理', cols: ['账号', '角色', '状态'], actions: 'sys-account' },
...
const app = createApp({
  setup() {
    ...
    return {
      ...
      tableData, userList, CONFIG_FIELDS, navigate, onRoleChange, fire, notify,
      ...
      memoirAnswersUser, memoirAnswersUserId, memoirAnsPage, memoirAnsFilter, memoirAnswerRows, pagedMemoirAnswers,
    }
  },
  template: `
    <div class="admin-root" v-if="true">
      ...
      <header class="top-header">
        ...
        <div class="header-right">
          <el-badge :value="3">
            <el-button text @click="notify">消息中心</el-button>
          </el-badge>
          <el-dropdown>
          ...
          <el-dropdown>
            <el-button text>全局系统配置</el-button>
            <template #dropdown>
              <el-dropdown-menu>
      ...
      <div class="admin-body">
        ...
        <main class="main-content">
          ...
          <div v-else-if="isMemoirManage" class="page-card">
            <h2 class="page-title">{{ pageMeta.title }}</h2>
            <p class="page-desc">状态列展示答题进度；点击用户名查看 <strong>1000</strong> 题回答精要（每题最多 1000 字）。</p>
            <div class="filter-bar">
              <el-input v-model="memoirManageKeyword" placeholder="手机号 / 用户名" style="width:220px" clearable />
          ...
          <div v-else-if="isMemoirAnswers && memoirAnswersUser" class="page-card">
            ...
            <p class="page-desc">已答 {{ memoirAnswersUser.answeredCount }} / {{ MEMOIR_Q_TOTAL }} 题，每题精要最多 1000 字。</p>
            <el-select v-model="memoirAnsFilter" style="width:120px;margin-bottom:12px">
              <el-option label="全部" value="all" />
              <el-option label="已回答" value="answered" />
              <el-option label="未回答" value="unanswered" />
            </el-select>
            <el-table :data="pagedMemoirAnswers" stripe border max-height="520">
              <el-table-column prop="id" label="题号" width="72" />
              <el-table-column prop="category" label="分类" width="100" />
              <el-table-column prop="question" label="题目" min-width="180" show-overflow-tooltip />
              <el-table-column label="回答精要" min-width="240">
                <template #default="{ row }">
                  <span v-if="row.essence">{{ row.essence }}</span>
